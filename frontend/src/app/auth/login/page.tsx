...
const slides = [
  ...
  { id: 2, src: "/auth-2.jpg", title: "Trải nghiệm hành trình tuyệt vời cùng chúng tôi" },
  { id: 3, src: "/auth-3.jpg", title: "Đặt vé nhanh chóng, an toàn và tiết kiệm" },
];
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      {/* Container chính: items-stretch đảm bảo 2 cột bằng chiều cao nhau */}
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl flex overflow-hidden items-stretch min-h-[650px]">
        ...
        <div className="w-1/2 hidden md:flex flex-col relative overflow-hidden">
          {slides.map((slide, index) => (
            ...
            >
              ...
              {/* Lớp nội dung (Chữ) thay đổi theo slide */}
                ...
                <h1 className="text-4xl font-extrabold leading-tight drop-shadow-2xl">
                  {slide.title.split('<br />').map((text, i) => (
                    <React.Fragment key={i}>
                      {text}
                      {i === 0 && slides[0].title.includes('<br />') && <br />}
                    </React.Fragment>
                  ))}
                  {/* Nếu title không có <br /> thủ công, render trực tiếp */}
                  {!slide.title.includes('<br />') && slide.title}
                </h1>
              </div>
            </div>
          ))}
          
          {/* Các dấu chấm trang trí có chức năng click chuyển slide */}
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  name="email"
                  required
                  className="appearance-none block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 sm:text-sm placeholder:text-gray-400"
              ...
              <div>
                ...
                {/* ĐÃ HIỆN LẠI VÀ TRỎ ĐÚNG ĐƯỜNG DẪN */}
                <Link href="/auth/forgot-password" title="Quên mật khẩu" className="text-sm font-semibold text-blue-600 hover:text-blue-500">
                    Quên mật khẩu?
                </Link>
                </div>

              <button type="submit" disabled={loading} className="w-full flex justify-center py-4 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-blue-400 transition-all duration-200 shadow-lg shadow-blue-200 cursor-pointer">
                {loading ? 'Đang xử lý...' : 'Đăng nhập'}
              </button>
              
              <div className="mt-8 text-center">
                <p className="text-sm text-gray-600 mb-5">Hoặc đăng nhập bằng</p>
                <div className="flex justify-center space-x-5">
                  <button type="button" className="w-12 h-12 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-gray-50 border border-gray-100 transition-transform hover:scale-110 active:scale-95 cursor-pointer">
                    <svg width="20" height="20" viewBox="0 0 48 48">
                      ...
                      <path fill="#4285F4" d="M46.64 24.55c0-1.65-.15-3.23-.42-4.75H24v9h12.75c-.55 2.89-2.2 5.34-4.71 6.99l7.33 5.69C43.64 37.1 46.64 31.39 46.64 24.55z"/>
                      <path fill="#FBBC05" d="M10.54 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24s.92 7.54 2.56 10.78l7.98-6.19z"/>
                      <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.33-5.69c-2.2 1.47-5.01 2.35-8.56 2.35-6.26 0-11.57-4.22-13.46-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
                    </svg>
                  </button>
                  <button type="button" className="w-12 h-12 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-gray-50 border border-gray-100 transition-transform hover:scale-110 active:scale-95 cursor-pointer">
                    <svg width="20" height="20" viewBox="0 0 20 20">
                      ...
                      <path fill="#fff" d="M13.26 12.89l.443-2.89H10.93V8.127c0-.791.387-1.562 1.63-1.562h1.26v-2.46s-1.144-.195-2.238-.195c-2.285 0-3.777 1.384-3.777 3.89V10H6.719v2.89h2.54v6.988C9.752 19.957 10 20 10 20c.248 0 .493-.043.719-.122v-6.99h2.33z"/>
                    </svg>
                  </button>
                  <button type="button" className="w-12 h-12 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-gray-50 border border-gray-100 transition-transform hover:scale-110 active:scale-95 cursor-pointer">
